package org.jetbrains.kotlin.konan.interop.gen.wasm

import org.jetbrains.kotlin.konan.interop.gen.wasm.idl.*
...
fun Operation.generateKotlin(parent: Interface): String {
    ...
    val wasmArgList = (wasmReceiverArgs(parent) + args.map{it.wasmMapping()} + wasmReturnArg).joinToString(", ")
...
fun processIdlLib(args: Array<String>) {
    ...
    File(ktGenRoot, "kotlin_stubs.kt").writeLines(listOf(generateKotlin(arguments.pkg!!, idl)))
    File(nativeLibsDir, "js_stubs.js").writeLines(listOf(generateJs(idl)))
}
